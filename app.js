...
app.get('/books/:id', (req, res, next) => {
  try
  {
      ...
      //Promise section has its own catch() to handle errors generated within
      .catch((err) => {
        res.send(500)
      });
  }
...
/* // 6th Route:  Define route for 'update' (POST - send information)
                  ...
                  Sequelize validators instead of express-validator */
...
app.post('/books/:id', (req, res, next) => {
  try
  {
    ...
    //Give promise section its own catch() to handle errors generated within
    //If Sequelize error, render the update-book page again, else fall into next .catch(err) - throw err;
    .catch((err) => {
      if (err.name === "SequelizeValidationError") {
...
try
  {
      ...
      Book.findAll({
        where: {
          [Op.and]: {
            title: {
              //Op.like already case-insensitive
